...
- Workspace environment source: `.codex/config.json` -> `env_file_source` and `environment.env_file_source`
- Deploy contract environment source: `.jarvis/deploy_contract.json` -> `env_file_source` and `environment.env_file_source`
- Contract consistency gate: `bash .codex/hooks/preflight.sh` and `node .codex/scripts/repo_operating_system_report.js --check` validate that `env_file_source` is present and aligned across workspace root, workspace environment, deploy root, deploy environment, verification, and production-profile contracts.
